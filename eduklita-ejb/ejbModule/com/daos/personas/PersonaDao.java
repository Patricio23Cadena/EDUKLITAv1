...
public class PersonaDao {
	...
	public Persona buscarCod(Persona us) {
		Persona usuario = null;
		String consulta;
		try {
			consulta = "SELECT u FROM Persona u WHERE u.id_usuario = ?1";
			Query query = em.createQuery(consulta);
			query.setParameter(1, us.getId_usuario());
	...
	public List<Persona> cambioContra(int id) {
		return em.createQuery("SELECT p FROM Persona p WHERE p.id_usuario = "+id).getResultList();
	}
